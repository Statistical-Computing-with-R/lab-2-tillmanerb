...
```
Reflection: I originally specified the relative filepath without the here package. To be consistent with the practices in the class, I read the data in using the here package.

**2. What are the dimensions (# of rows and columns) of these data?**

```{r}
cat('Number of rows: ', nrow(surveys))
...
cat('\nNumber of columns: ', ncol(surveys))

```
Reflection: I wrote nrows twice whereas one should have been ncols. This is because I found copy-pasted a previous line of code, failing to double check that I had altered it to get the number of columns. In the future, I should take more caution when copy-pasting lines of code.
...
facet_wrap(~species) + 
labs(x = "Species weight (grams)",
     y = "",
     title = "Species weight versus hindfoot length for species in Portal, AZ",
...
geom_boxplot(outliers = FALSE) +
labs(subtitle = "Species of Animal",
     x = "Weight (g)",
     y = ""
) +
#https://stackoverflow.com/questions/1330989/rotating-and-spacing-axis-labels-in-ggplot2
theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
Reflection: When flipping my boxplot, I did not double check my axes labels. This is a similar issue to above because I copy-pasted my code from earlier without thoroughly vetting it. I should exercise more caution here. I relabeled the axes and added units, which I was previously missing but which are necessary for the viewer of the plot to better understand the data. Furthermore, I put my boxplot over the geom_jitter as it was previously being obscured by the jitter data.
