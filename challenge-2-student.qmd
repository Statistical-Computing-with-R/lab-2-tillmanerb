...
labs(x = "Weight (grams)",
     y = "Species of Animal")
```
Reflection: I originally did not specify labels for my axes, and therefore they were labelled as the variable names. However, it's important that the viewer of this plot understands what's going on at a glance, and therefore it's important to add units and specify what 'species' exactly indicates.
...
```
Reflection: Initially, I only labelled each genus once in the boxplot. However, to make it abundantly clear to the user which genus each species represents, it's good to label each boxplot. Therefore, I added remaining labels.
